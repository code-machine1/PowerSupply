...
uint8_t BSP_W25Qx_Erase_Block64K(uint8_t block_number)
{
    uint8_t cmd[4];
    uint32_t tickstart = HAL_GetTick();
    cmd[0] = SECTOR_ERASE_64K_CMD;
    cmd[1] = (block_number * 64 * 1024) >> 16;
    cmd[2] = (block_number * 64 * 1024) >> 8;
    cmd[3] = (block_number * 64 * 1024) >> 0;
...
uint8_t BSP_W25Qx_Write_Page(uint8_t *data,uint16_t page_numbere)
{
    ...
    uint32_t tickstart = HAL_GetTick();
    cmd[0] = PAGE_PROG_CMD;
    cmd[1] = (page_numbere *256) >> 16;
    cmd[2] = (page_numbere *256) >> 8;
    cmd[3] = (page_numbere *256) >> 0;
    while(BSP_W25Qx_GetStatus() == W25Qx_BUSY)
        ;
    {
        if((HAL_GetTick() - tickstart) > W25Q128FV_SECTOR_ERASE_MAX_TIME)
    ...
    if (HAL_SPI_Transmit(&hspi2,data, 256, W25Qx_TIMEOUT_VALUE) != HAL_OK)
    {
        return W25Qx_ERROR;
    }
    W25Qx_Disable();
...
uint8_t BSP_W25Qx_Read_Data(uint8_t *data,uint32_t addr,uint32_t datalen)
{
    ...
    if (HAL_SPI_Receive(&hspi2, data,datalen,W25Qx_TIMEOUT_VALUE) != HAL_OK)
    {
        return W25Qx_ERROR;
    }
    W25Qx_Disable();
